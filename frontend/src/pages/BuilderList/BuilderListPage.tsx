...
const BuilderListPage: React.FC = () => {
  ...
  const fetchBuilders = useCallback(async () => {
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erro ao carregar construtoras';
  ...
  if (error) {
    return (
      <div className="text-center py-12">
        ...
        <h3 className="text-lg font-medium text-gray-900 mb-2">Erro ao carregar construtoras</h3>
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-gray-900">Construtoras</h1>
        <p className="mt-2 text-gray-600">
          Conheça as empresas parceiras e seus empreendimentos
        </p>
      </div>
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {builders.map((builder) => (
              <div key={builder.id} className="card hover:shadow-lg transition-shadow duration-200">
                <div className="card-body">
                  {/* Video/Photo preview */}
                  {builder.video ? (
                    ...
                    <div className="relative mb-4 aspect-video bg-gray-100 rounded-lg overflow-hidden">
                      <img
                        src={apiClient.getMediaUrl(builder.photos[0].image)}
                        alt={`${builder.name} - Foto 1`}
                        className="w-full h-full object-cover"
                      />
                      {builder.photos.length > 1 && (
                        <div className="absolute bottom-2 right-2 bg-black bg-opacity-70 text-white text-xs px-2 py-1 rounded">
                          +{builder.photos.length - 1} fotos
                        </div>
                      )}
                    </div>
                  ) : (
